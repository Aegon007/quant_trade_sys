...
from slack_command_parser import ParsedSlackCommand, parse_slack_command


COMMAND_AUDIT_FILE = "command_audit.jsonl"
...
def _format_watchlist_message(data, snapshot) -> str:
    ...
    for watch in watchlist:
        ...
        lines.append(
            "- "
            f"{watch['symbol']} | 目标 {_format_currency(watch.get('target_buy'))} | "
            f"最新 {_format_currency(watch.get('last_price'))} | "
            f"备注 {notes}"
        )
    account_line = _account_summary_line(snapshot)
    if account_line:
...
def _format_status_message(data, symbol: str) -> str:
    ...
    if watch is not None:
        return "\n".join(
            [
                f"{symbol} 当前在关注列表中",
                f"- 目标买入价: {_format_currency(watch.get('target_buy'))}",
...
def execute_slack_command(text) -> CommandExecutionResult:
    ...
    try:
        ...
        if command.name in {"SELL_ALL", "MOVE_TO_WATCH"}:
        ...
        if command.name == "MOVE_TO_HOLDING":
            ...
            action = pactions.buy_symbol(command.symbol, shares)
            updated_data = du.load_data()
            account_line = _account_summary_line(_load_snapshot(data=updated_data))
            message = (
