...
def recommend_position_action(
    ...
    allocation_regime_name = ""
    allocation_multiplier = 1.0
    if allocation_regime is not None:
        allocation_regime_name = str(getattr(allocation_regime, "regime", "") or "").upper()
        allocation_multiplier = float(getattr(allocation_regime, "risk_multiplier", 1.0) or 1.0)
        allocation_multiplier = max(0.0, allocation_multiplier)
        if bool(getattr(allocation_regime, "block_new_buys", False)):
            gate_block_buys = True
        effective_max_weight = min(1.0, effective_max_weight * max(allocation_multiplier, 0.25))
    ...
    if allocation_regime_name:
        reason_parts.append(f"当前仓位节奏: {allocation_regime_name}。")
    if signal_approval.blocked and signal_approval.reason:
        reason_parts.append(signal_approval.reason)
    if gate_block_buys:
