...
def _resolve_total_and_invested_dollars(
    *,
    account: Mapping,
    cash_available: Optional[float],
    current_value: float,
    current_invested_dollars: Optional[float],
) -> tuple[float, float]:
    legacy_total_capital = max(_float(account.get("total_capital")), 0.0)

    if current_invested_dollars is not None:
        invested_dollars = max(_float(current_invested_dollars), current_value, 0.0)
    elif cash_available is not None:
        # Prefer dynamic capital sizing (cash + known invested value). Legacy total_capital
        # is only used as a coarse fallback when this symbol already has position value.
        if legacy_total_capital > 0 and current_value > 0:
            invested_dollars = max(legacy_total_capital - cash_available, current_value, 0.0)
        else:
            invested_dollars = max(current_value, 0.0)
    else:
        invested_dollars = max(current_value, 0.0)

    if cash_available is not None:
        total_capital = cash_available + invested_dollars
    elif legacy_total_capital > 0:
        total_capital = max(legacy_total_capital, invested_dollars)
    else:
        total_capital = invested_dollars

    return total_capital, invested_dollars
...
def recommend_allocation(
    *,
    ...
    signal = normalize_signal_direction(signal_approval.approved_signal)
    ...
    if current_price is None or _float(current_price) <= 0:
    ...
    total_capital, invested_dollars = _resolve_total_and_invested_dollars(
        account=account,
    ...
    risk_multiplier = 1.0
    allocation_block_new_buys = False
    ...
    if allocation_regime is not None:
        allocation_multiplier = _float(getattr(allocation_regime, "risk_multiplier", 1.0), 1.0)
        risk_multiplier *= max(0.0, allocation_multiplier)
        allocation_block_new_buys = bool(getattr(allocation_regime, "block_new_buys", False))
        allocation_regime_name = str(getattr(allocation_regime, "regime", "") or "").upper()
        allocation_reasons = list(getattr(allocation_regime, "reasons", []) or [])
        if allocation_regime_name:
            reason_parts.append(f"仓位节奏: {allocation_regime_name}")
        if allocation_reasons:
            reason_parts.append("闭环依据: " + " ".join(allocation_reasons))
        if allocation_block_new_buys:
            reason_parts.append("仓位节奏规则暂停新增仓位。")

    if allocation_block_new_buys and signal == "BUY":
        signal = "HOLD"

    if signal != "BUY":
        reason = "当前不是买入信号，暂不新增仓位。"
        if signal_approval.blocked and signal_approval.reason:
            reason = signal_approval.reason
        elif reason_parts:
