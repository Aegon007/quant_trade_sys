...
- 默认生产方向必须使用真实时间序列 foundation backend。当前首选为 Chronos-2；
  未安装依赖或权重时系统必须显示 `MODEL_UNAVAILABLE`，不能用 proxy 伪装模型输出。
- Foundation job 必须持续归档实际读取到的历史 OHLCV 观测数据，并按配置保留一段时间，
  作为未来 Chronos-2 fine-tune / LoRA 适配的数据基础。
...
## 10. V3.1 策略身份重构：从“四不像”收敛为三层交易系统

### 10.1 新系统定位

系统不再尝试同时覆盖趋势跟随、均值回归、统计套利和高频交易。当前个人系统的正式定位是：

`中长期个人交易辅助系统 = 核心 ETF 配置 + 卫星仓趋势/基本面雷达 + 风险纪律层`
...
- 不做日内自动交易
- 不把统计套利作为主交易引擎
- 不让新闻、LLM 或研究摘要直接生成买卖动作

### 10.2 四类常见量化策略在本系统中的角色

| 策略类型 | 是否作为主线 | 在本系统中的定位 |
| --- | --- | --- |
| 趋势跟随 | 是 | 卫星仓雷达的主逻辑，用来发现中长期上涨趋势和趋势破坏 |
| 均值回归 | 部分使用 | 只作为 ETF 定投、回调买入、减仓后重新进入的价格辅助，不单独驱动重仓 |
| 统计相关性 / 套利 | 周末研究使用 | 用来识别持仓冗余、行业拥挤、低相关强势资产和下周研究线索，不直接生成交易指令 |
| 高频交易 | 否 | 暂不进入系统范围，避免基础设施、延迟、滑点和风控复杂度失控 |

### 10.3 统一决策层

所有页面、夜间报告和 Slack / Email 摘要最终都应该引用同一个 `final_decision_snapshot`。它必须回答：

- 今天是否有可执行动作
- 核心 ETF 是否应该加仓、暂停、调仓或等待
- 卫星仓是否有 Top 3 强候选
- 风险纪律层是否允许行动
- 周末相关性研究是否只提供线索，还是发现了需要降低集中度的风险
...
2. 核心 ETF 引擎负责长期主仓配置。
3. 卫星仓雷达只负责少量高质量机会。
4. 相关性研究和 LLM 新闻解释只能作为 evidence，不直接越权生成交易。

### 10.4 周末相关性研究的正式边界

周末可以运行更重的统计相关性与跨资产数据挖掘研究，因为它需要扫描更多标的和更长历史窗口。研究宇宙可以扩展到几千甚至上万只股票、ETF、黄金 ETF、商品 ETF、债券 ETF、反向/杠杆 ETF。输出包括：

- 高相关资产对
- 当前持仓冗余
- 低相关且相对强势的候选资产
- 简单相关性簇，用来识别主题拥挤和相似风险暴露
- 组合集中度风险线索
- 下周需要人工重点研究的候选池变化

这些结果必须标记为 `RISK_AND_OPPORTUNITY_CLUES`，不能直接变成买入或卖出建议。真正交易动作仍需经过核心 ETF / 卫星仓评分和风险纪律层。
...
Dashboard 首屏必须先显示统一策略视图，而不是把多个模块的结论平铺给用户自行解释。
...
Slack / Email 也必须使用同一套结论，避免网页显示买入、Slack 显示无动作、报告又显示观望的冲突。发送到 Slack / Email 前，结构化报告应优先交给 LLM 通知摘要层改写成自然中文；如果 LLM 不可用，才退回结构化报告。

### 10.6 已落地状态

本轮升级已经把以下能力纳入代码主链路：

- `strategy_identity`：给信号标注策略来源和是否允许直接交易
- `decision_engine`：生成统一最终决策快照
- `correlation_research`：周末相关性研究输出风险与机会线索
- `weekend_universe`：合并持仓、关注、核心 ETF、卫星池和周末专用 universe
- Dashboard：增加统一策略视图区块
- Weekend Research 页面：展示周末长任务状态、研究宇宙、算法阶段和相关性/机会线索
- Nightly report：加入统一决策和相关性研究摘要
- Notification digest：Slack / Email 发送前优先调用 LLM 生成中文可读摘要
- API：暴露 `final-decision`、`weekend-research` 和 `weekend-correlation` 快照

后续改进应优先围绕“信号一致性、数据健康、交易后反馈、模型证据质量”展开，而不是继续堆叠更多互相独立的策略。
